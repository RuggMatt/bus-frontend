...
import { getElectricBusStatsHourly, getElectricBusStatsDaily } from '../api/index.js';
...
const renderHourlyChart = (data) => {
  ...
  hourlyChartInstance = new Chart(hourlyChartRef.value, {
    ...
    data: {
      labels,
      datasets: [{ label: 'Unique Electric Buses per Hour', data: data.map(i => i.unique_bus_count), backgroundColor: 'rgba(54, 162, 235, 0.5)' }],
    },
    options: {
      responsive: true,
      plugins: { legend: { display: false }, title: { display: true, text: 'Hourly Unique Electric Bus Count' } },
      scales: {
...
const renderDailyChart = (data) => {
  ...
  dailyChartInstance = new Chart(dailyChartRef.value, {
    ...
    data: {
      labels,
      datasets: [{ label: 'Unique Electric Buses per Day', data: data.map(i => i.unique_bus_count), backgroundColor: 'rgba(75, 192, 120, 0.5)' }],
    },
    options: {
      responsive: true,
      plugins: { legend: { display: false }, title: { display: true, text: 'Daily Unique Electric Bus Count' } },
      scales: {
...
// --- Lifecycle & watchers ---
onMounted(async () => {
  allHourlyData.value = await getElectricBusStatsHourly();
  renderHourlyChart(getContinuousHourlyData(allHourlyData.value, selectedHourlyRange.value));

  allDailyData.value = await getElectricBusStatsDaily();
});
...
watch(activeTab, async (tab) => {
  if (tab === 'daily' && allDailyData.value.length) {
    await nextTick();
